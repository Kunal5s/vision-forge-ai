...
export async function getArticles(category: string): Promise<Article[]> {
    ...
    try {
        ...
        // If file doesn't exist or is empty, it will be generated by the user running the app locally once,
        // or by the daily CRON job. The page itself will NOT trigger generation anymore to prevent build failures.
        console.warn(`No articles file found for category "${category}" at path: ${filePath}. Please generate it or wait for the CRON job.`);
        return [];
