    ...
    return (
        <div className="w-full bg-background border-b">
            <ScrollArea className="w-full whitespace-nowrap no-scrollbar" ref={scrollContainerRef}>
                <nav className="container mx-auto flex items-center justify-start gap-4 px-4 h-12">
                {navLinks.map((link) => (
                    <Link
                        key={link.href}
                        href={link.href}
                        className={cn(
                            "text-sm font-medium text-muted-foreground transition-colors hover:text-foreground shrink-0",
